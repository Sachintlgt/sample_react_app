...
import {
    ...
    countryListResponse,SUCCESS_COUNTRY_LIST,ERROR_COUNTRY_LIST,COUNTRY_LIST
} from './auth.action';
import {
    loginApi, forgotPasswordApi, resetPasswordApi, verifyTokenApi, registrationApi,
    logoutApi, resendEmailVerificationApi ,countryListApi
} from '../../../api/index';
...
// countryListRequest
function* countryListRequest(data) { 
    let getData = yield countryListApi(data);
    if (getData.success && _.has(getData, 'data.data')) {
        yield put(countryListResponse(SUCCESS_COUNTRY_LIST, getData.data));
    } else {
        yield put(countryListResponse(ERROR_COUNTRY_LIST, getData.data));
    }
}

 export function* countryListWatcher() { 
     yield takeLatest(COUNTRY_LIST, countryListRequest);
 }