...
import { userLogin, resendEmailVerification,countryList } from '../../../../duck/auth/auth.action';
...
export const NewRegister = props => {
    ...
    const countryData =  useSelector(state => state.auth.countryListData);
    // Set initial State Value  
    const [state, setState] = useState({
        email: "", password: "", confirmPassword: "",country: "US",countryCls: '', countryErr: '', emailCls: '', emailErr: '', passwordCls: "", passwordErr: "", confirmPasswordCls: "", confirmPasswordErr: "",
        correctInput: constants.RIGHT_INPUT, wrongInput: constants.WRONG_INPUT
    });
    const [loader, setLoader] = useState(false);
    const [serviceMessage, setServiceMessage] = useState('');
    const [selectedCountry, setSelectedCountry] = useState({value: 'US', label: 'USA'});


     // On Load Get countryList
     useEffect(() => {
        //setLoader(true)
            ...
            if (registerData && _.has(registerData, 'data') && registerData.success === true) {
                async function setTokenAfterRegistration() {
                    ...
                    let userData = {
                        ...
                        planData: registerData && registerData.user_preferences ? registerData.user_preferences[0] : '',
                    }
    ...
    const saveUserData = () => {
        ...
        let email = state.email,country=state.country, password = state.password, confirmPassword = state.confirmPassword, emailCls = success,
            passwordCls = success, emailErr = '',countryErr = '',countryCls = '', passwordErr = '', confirmPasswordCls = success, confirmPasswordErr = '', getError = false;
        ...
        setState({ ...state, emailCls, emailErr, passwordCls, passwordErr, confirmPasswordCls, confirmPasswordErr,countryErr,countryCls })

        if (getError === false && emailErr === '' && passwordErr === '' && confirmPasswordErr === '') {
            setLoader(true)
            dispatch(registration({ email, password, password_confirmation: confirmPassword ,country  }))
        }
    }
                            ...
                            <div className="col-xl-5 col-lg-5 col-md-6 col-sm-8">
                                <div className="card">
                                    <div className="card-header">
                                        <h5>FREE Sign-up with MyBizzHive</h5>
                                    ...
                                    <div className="card-body">
                                        ...
                                        <div className={"form-group floating-label " + state.countryCls}> 
                                        <div className="floating-label phone-type">                                
                                            <Select
                                                styles={selectStyle}
                                                ...
                                                options={   _.map(countryData.data, (val, ind) => {
                                                            return  { value:val.country_code, label: val.country_name }
                                                        })}
                                                placeholder="Country"
                                                onChange={(option)=>{ setCountry(option.value); setSelectedCountry(option)}}
                                            />
                                    </div>
                                            {state.countryErr ? <span className="errorValidationMessage"> {state.countryErr}</span> : ''}
                                        </div>                                        
                                        <p>By signing up, you agree to MyBizzHive <a href={constants.FRONT_URL+'/terms-and-conditions.html'} rel="noopener noreferrer" target="_blank">Terms & Conditions</a></p>
                                        <div className="form-group">
                                            <button type="button" onClick={() => saveUserData()} className="btn btn-block btn-primary">Sign up</button>
                                        </div>
                                        <p className=" text-center">Already a MyBizzHive member?  <Link to={LOGIN}>Sign In</Link></p>
